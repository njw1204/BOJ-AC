...
int N;
vector<pi> arr;
int computedArr[100005];

int fenwick[100005];
int result[100005][2];

void add(int target, int val) {
...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        result[i][0] = sum(N) - sum(computedArr[i]);
        add(computedArr[i], 1);
    }
    ...
    for (int i = N; i >= 1; i--) {
        result[i][1] = sum(N) - sum(computedArr[i]);
        add(computedArr[i], 1);
    }

    int ans = 0;
    for (int i = 1; i <= N; i++) {
        if (result[i][0] > result[i][1] * 2 || result[i][1] > result[i][0] * 2) {
            ans++;
        }
    }
