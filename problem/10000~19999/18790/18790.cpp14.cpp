...
int main() {
    ...
    for (int i = 1; i < N * 2 - 1; i++) {
        ...
        for (int j = 1; j <= N; j++) {
            for (int k = 0; k < N; k++) {
                int beforeMod = (k - arr[i] + N) % N;

                if (dp[i - 1][j][k]) {
                    dp[i][j][k] = 's';
                }
                else if (dp[i - 1][j - 1][beforeMod]) {
                    dp[i][j][k] = '+';
                }
            }
        }
    ...
    if (dp[N * 2 - 2][N][0]) {
        int idx = N * 2 - 2, cnt = N, targetMod = 0;
        while (cnt) {
            int beforeMod = (targetMod - arr[idx] + N) % N;
            if (dp[idx][cnt][targetMod] == '+') {
                cout << arr[idx] << ' ';
            ...
            else if (dp[idx][cnt][targetMod] == 's') {
                idx--;
            }
            else {
                cout << arr[idx];
