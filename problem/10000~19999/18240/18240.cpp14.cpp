...
queue<int> nodeOfDepth[300005];
int nowVal = 1;

void dfs(int root) {
    if (child[root][0]) dfs(child[root][0]);
    valOf[root] = nowVal++;
    if (child[root][1]) dfs(child[root][1]);
}

int main() {
    UNSYNC; READTXT;
    INPUT(N);
    nodeOfDepth[0].push(1);

    REP(i, N - 1) {
        DINPUT(x);
        if (nodeOfDepth[x - 1].empty()) {
            return cout << -1, 0;
        }

        int p = nodeOfDepth[x - 1].front();
