...
int main() {
    ...
    if (M < 1 || N > 300) return -1;
    if (K < 1 || N > 300) return -1;


    for (int i = 1; i <= N; i++) {
        DINPUT2(ch, po);

        if (ch < 1 || ch > 300) return -1;
        if (po < 1 || po > 300) return -1;

        for (int j = 1; j <= M; j++) {
            for (int k = 1; k <= K; k++) {
                if (j >= ch && k >= po) {
                    dp[i][j][k] = max(dp[i - 1][j][k], dp[i - 1][j - ch][k - po] + 1);
                }
                else {
                    dp[i][j][k] = dp[i - 1][j][k];
                }
