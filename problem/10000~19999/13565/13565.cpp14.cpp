...
void BFS(int si, int sj) {
  ...
  while (!Q.empty()) {
    ...
    for (int i = 0; i < 4; i++) {
      pair<int, int> next = { now.$1 + dx[i].$1, now.$2 + dx[i].$2 };
      if (next.$1 >= 1 && next.$1 <= M && next.$2 >= 1 && next.$2 <= N && field[next.$1][next.$2] == 0 && !visit[next.$1][next.$2]) {
        Q.push({ next.$1,next.$2 });
