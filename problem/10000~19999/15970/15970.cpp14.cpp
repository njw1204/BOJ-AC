...
ll ans;
int N;
vector<int> spot[100005];
vector<pair<int, int>> spotArr;
...
int main() {
  ...
  for (auto i : spotArr) {
    ...
    auto pos = lower_bound(ALL(spot[i.$2]), i.$1);
    if (pos == spot[i.$2].begin()) {
      ans += spot[i.$2][1] - i.$1;
    }
    else if (pos == spot[i.$2].end() - 1) {
      ans += i.$1 - *(spot[i.$2].end() - 2);
    }
    else {
      ans += min(*pos - *(pos - 1), *(pos + 1) - *pos);
    }
  }
