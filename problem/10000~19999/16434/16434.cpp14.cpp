...
bool solve(long long maxHp, long long atk) {
    long long nowHp = maxHp;

    for (auto& i : room) {
        if (get<0>(i) == 2) {
            atk += get<1>(i);
            nowHp = min(maxHp, nowHp + get<2>(i));
        }
        else {
            long long mobAtk = get<1>(i), mobHp = get<2>(i);
            long long cnt = mobHp / atk;
            if (mobHp % atk != 0) cnt++;
...
int main() {
    ...
    long long left = 1, right = 200000000000000000;
    while (left < right) {
        long long mid = (left + right) / 2;
        if (solve(mid, ATK)) right = mid;
        else left = mid + 1;
    }
