...
vector<base> cp(vector<base>& origin) {
    vector<base> asdf;
    for (auto i : origin) {
        asdf.pb(base((ll)round(i.real()) ? 1 : 0));
        if (asdf.size() >= 1000001) break;
    }
    return asdf;
}

vector<base> pow_fft(vector<base> a, int k) {
    vector<base> ans = a;
    k--;

    while (k) {
        if (k & 1) {
            vector<base> t = ans, t2 = a;
            multiply(t, t2);
            ans = cp(t);
        }
        k /= 2;
        
        vector<base> t = a, t2 = a;
        multiply(t, t2);
        a = cp(t);
    }
...
int main() {
    ...
    for (int i = 0; i <= 1000; i++) {
        a.pb(base((isExisted[i] ? 1 : 0)));
    }

    vector<base> ans = pow_fft(a, K);
