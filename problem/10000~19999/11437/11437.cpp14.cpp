...
int Solve(int a, int b) {
  ...
  if (height[a] < height[b])
    return dp[{a, b}] = Solve(min(a, parent[b]), max(a, parent[b]));
  else if (height[a] > height[b])
    return dp[{a, b}] = Solve(min(parent[a], b), max(parent[a], b));
  else
    return dp[{a, b}] = Solve(min(parent[a], parent[b]), max(parent[a], parent[b]));
}
