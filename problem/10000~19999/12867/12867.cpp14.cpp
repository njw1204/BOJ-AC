...
int main() {
  ...
  for (auto i : arr) indexSet.insert(i);

  int j = 0;
  for (auto i : indexSet) zip[i] = j++;

  for (int i = 0; i < j; i++) state.push_back(60);
  visit[state] = true;

  for (int i = 0; i < M; i++) {
    int pos = zip[arr[i]], going;
    if (dir[i] == '+') going = 1;
    else going = -1;
    state[pos] += going;
    if (visit[state]) return puts("0"), 0;
    else visit[state] = true;
  }
