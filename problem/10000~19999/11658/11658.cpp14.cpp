...
int N, M;
int table[1025][1025];
int fenwick[1025][1025];

void update(int si, int sj, int dx) {
  for (int i = si; i <= N; i += i & -i) {
    for (int j = sj; j <= N; j += j & -j) {
      fenwick[i][j] += dx;
    }
  }
}

int sum(int fi, int fj) {
  int s = 0;
  for (int i = fi; i > 0; i -= i & -i) {
    for (int j = fj; j > 0; j -= j & -j) {
      s += fenwick[i][j];
    }
  }
  return s;
...
int main() {
  ...
  REP(i, M) {
    DINPUT(op);
    if (op == 1) {
      DINPUT2(si, sj); DINPUT2(fi, fj);
      PRINT(sum(fi, fj) - sum(si - 1, fj) - sum(fi, sj - 1) + sum(si - 1, sj - 1));
    } else {
      DINPUT3(si, sj, c);
      update(si, sj, c - table[si][sj]);
      table[si][sj] = c;
    }
  }
  return 0;
