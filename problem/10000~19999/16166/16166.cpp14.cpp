...
void Dijkstra() {
  auto cmp = [](pair<int, int>& a, pair<int, int>& b) { return dist[a] > dist[b]; };
  ...
  while (!Q.empty()) {
    auto i = Q.top(); Q.pop();
    for (int j = 1; j <= N; j++) {
      if (find(ALL(yArr[j]), i.$1) == yArr[j].end()) continue;
      for (int k : yArr[j]) {
        if (i.$1 == k && i.$2 == j) continue;
        int bn = (i.$2 == j ? 0 : 1);
        if (!visit[{k, j}] || dist[i] + bn < dist[{k, j}]) {
          Q.push({ k, j });
          dist[{k, j}] = dist[i] + bn;
          visit[{k, j}] = true;
          if (k == E) {
            ans = min(ans, dist[{k, j}]);
          }
        }
      }
    }
...
int main() {
  ...
  REP(i, N) {
    ...
    REP(j, yCnt[i + 1]) {
      DINPUT(x);
      yArr[i + 1].pb(x);
      if (x == 0) initHosun.insert(i + 1);
    }
  }
  INPUT(E);
  if (E == 0) return cout << 0, 0;
