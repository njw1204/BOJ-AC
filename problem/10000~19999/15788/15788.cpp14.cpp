...
int main() {
  ...
  for (int i = 1; i <= N; i++) {
    for (int j = 1; j <= N; j++) {
      sumLine[i][0] += field[i][j];
      sumLine[i][1] += field[j][i];
    }
    if (i != pi) {
      if (peace == -1) peace = sumLine[i][0];
      else if (peace != sumLine[i][0]) return puts("-1"), 0;
    }
    if (i != pj) {
      if (peace == -1) peace = sumLine[i][1];
      else if (peace != sumLine[i][1]) return puts("-1"), 0;
    }
    sumCross[0] += field[i][i];
    sumCross[1] += field[i][N - i + 1];
  }

  if (sumCross[0] != peace && pi != pj) return puts("-1"), 0;
  if (sumCross[1] != peace && N - pi + 1 != pj) return puts("-1"), 0;

  if (pi == pj && N % 2 == 1 && pi == N / 2 + 1) {
    ll a = sumLine[pi][0], b = sumLine[pj][1], c = sumCross[0], d = sumCross[1];
    if (a != b || a != c || a != d || b != c || b != d || c != d) return puts("-1"), 0;
    return cout << peace - a, 0;
  }
  else if (pi == pj) {
    ll a = sumLine[pi][0], b = sumLine[pj][1], c = sumCross[0];
    if (a != b || a != c || b != c) return puts("-1"), 0;
    return cout << peace - a, 0;
  }
  else if (N - pi + 1 == pj) {
    ll a = sumLine[pi][0], b = sumLine[pj][1], c = sumCross[1];
    if (a != b || a != c || b != c) return puts("-1"), 0;
    return cout << peace - a, 0;
  }
  else {
    ll a = sumLine[pi][0], b = sumLine[pj][1];
