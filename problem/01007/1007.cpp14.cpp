...
#define FILL(con,x) for(auto& acm_icpc:(con))acm_icpc=(x);
...
ll ans;
vector<pair<ll, ll>> spot;

void BackTrack(int start, int fin, int remain, pair<ll, ll> tempSum){
  if (fin - start < remain) return;

  if (remain == 1){
    for (int i = start; i < fin; i++){
      ll tempX = tempSum.$1 - spot[i].$1 * 2;
      ll tempY = tempSum.$2 - spot[i].$2 * 2;
      ans = MIN(ans, tempX*tempX + tempY*tempY);
    }
  }
  else {
    for (int i = start; i < fin; i++){
      ll tempX = tempSum.$1 - spot[i].$1 * 2;
      ll tempY = tempSum.$2 - spot[i].$2 * 2;
      BackTrack(i + 1, fin, remain - 1, {tempX,tempY});
    }
  }
}
