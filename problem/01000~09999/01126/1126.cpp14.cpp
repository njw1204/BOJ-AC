...
int main() {
    ...
    for (int i = 1; i < N; i++) {
        for (int j = -500000; j <= 500000; j++) {
            ...
            int nowHeight = arr[i];
            int diff1 = j - nowHeight, diff2 = j + nowHeight;

            if (diff1 + BASE >= 0 && dp[i - 1][diff1 + BASE] != -1) {
                dp[i][j + BASE] = max(dp[i][j + BASE], dp[i - 1][diff1 + BASE] + nowHeight);
            }

            if (diff2 + BASE <= 1000000 && dp[i - 1][diff2 + BASE] != -1) {
                dp[i][j + BASE] = max(dp[i][j + BASE], dp[i - 1][diff2 + BASE]);
            }
        }
    }

    if (dp[N - 1][BASE] < 1) return cout << -1, 0;
    return cout << dp[N - 1][BASE], 0;
}