...
int N, M, C = 0;
set<int> before[32005];
set<int> after[32005];
set<pair<int,int>> linkCnt;
map<int, bool> already;

/*
void DFS(int start) {
  if (already[start]) return;
  C++;
  cout << start << ' ';
  already[start] = true;
  for (auto i : after[start]) {
    before[i].erase(start);
    if (before[i].empty()) DFS(i);
  }
}
*/
...
int main() {
  ...
  for (int i = 1; i <= N; i++) {
    linkCnt.insert({ before[i].size(), i });
  }

  while (!linkCnt.empty()) {
    auto i = *linkCnt.begin();
    linkCnt.erase(linkCnt.begin());
    cout << i.$2 << ' ';
    for (auto j : after[i.$2]) {
      linkCnt.erase({ before[j].size(),j });
      before[j].erase(i.$2);
      linkCnt.insert({ before[j].size(),j });
    }
  }
}
