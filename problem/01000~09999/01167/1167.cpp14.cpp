...
ll BFS(int start, int type) {
  queue<int> Q;
  INIT(visit);
  Q.push(start);
  visit[start] = 1;
  ll answer = start, longDistance = 1;
  ...
  while (!Q.empty()) {
    ...
    for (auto j : link[i]) {
      if (!visit[j.$1]) {
        Q.push(j.$1);
        visit[j.$1] = visit[i] + j.$2;
        if (visit[j.$1] > longDistance) {
          longDistance = visit[j.$1];
          answer = j.$1;
        }
      }
    }
  ...
  if (type == 1) return answer;
  else return longDistance - 1;
}
