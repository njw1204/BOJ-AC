...
int main() {
    ...
    for (int i = 1; i <= 99; i++) {
        for (int j = 0; j <= 99; j++) {
            ...
            if ((spoint + 1) % 100 == epoint) {
                dp[spoint][epoint] = (int)has[spoint][epoint];
                ans = max(ans, dp[spoint][epoint]);
                continue;
            }

            if ((spoint + 2) % 100 == epoint) {
                dp[spoint][epoint] = max(dp[spoint][(spoint + 1) % 100], dp[(spoint + 1) % 100][epoint]);
                dp[spoint][epoint] = max(dp[spoint][epoint], (int)has[spoint][epoint]);
                ans = max(ans, dp[spoint][epoint]);
                continue;
            }
            ...
            for (int k = spoint; k != epoint; k++) {
                ...
                dp[spoint][epoint] = max(dp[spoint][epoint], dp[spoint][k] + dp[(k + 1) % 100][epoint]);
            }

            dp[spoint][epoint] = max(dp[spoint][epoint], dp[(spoint + 1) % 100][(epoint - 1 + 100) % 100] + (int)has[spoint][epoint]);
            ans = max(ans, dp[spoint][epoint]);
        }
    }
