...
int blockCnt[2][500001], ans = INT_MAX, ansCnt;
...
int main() {
  ...
  REP(i, n) {
    DINPUT(input);
    if (i % 2) blockCnt[0][h - input + 1]++;
    else blockCnt[1][input]++;
  }

  for (int i = 2; i <= h; i++)
    blockCnt[0][i] += blockCnt[0][i - 1];

  for (int i = h - 1; i >= 0; i--)
    blockCnt[1][i] += blockCnt[1][i + 1];

  for (int i = 1; i <= h; i++) {
    int upBreak = blockCnt[0][i];
    int downBreak = blockCnt[1][i];

    if (upBreak + downBreak < ans) {
      ans = upBreak + downBreak;
      ansCnt = 1;
    } else if (upBreak + downBreak == ans) ansCnt++;
  }
