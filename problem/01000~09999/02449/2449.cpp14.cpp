...
ll solve(int s, int e, int color) {
    ll tans = INT_MAX;
    if (dp[s][e][color] < INT_MAX) return dp[s][e][color];

    bool check = false;
    for (int i = s; i <= e - 1; i++) {
        if (arr[i] != arr[i + 1]) {
            check = true;
            tans = min(tans, solve(s, i, color) + solve(i + 1, e, color));

            if (arr[s] != color) {
                tans = min(tans, solve(s, i, arr[s]) + solve(i + 1, e, color) + 1);
                tans = min(tans, solve(s, i, arr[s]) + solve(i + 1, e, arr[s]) + 1);
            }

            if (arr[e] != color) {
                tans = min(tans, solve(s, i, color) + solve(i + 1, e, arr[e]) + 1);
                tans = min(tans, solve(s, i, arr[e]) + solve(i + 1, e, arr[e]) + 1);
            }
        }
    }

    if (!check) {
        if (arr[s] == color) tans = 0;
        else tans = 1;
    }

    return dp[s][e][color] = tans;
}
