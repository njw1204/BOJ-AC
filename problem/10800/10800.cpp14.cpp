...
map<int, int> point[200005];
unordered_map<int, int> pointSum[200005];
...
int main(){
  ...
  for (int k = 0; k <= N; k++) {
    int lastMax = 0;
    for (auto i : point[k]) {
      pointSum[k][i.$1] = pointSum[k][lastMax] + point[k][lastMax] * lastMax;
      lastMax = i.$1;
    }
  }

  for (auto i : query) {
    PRINT(pointSum[0][i.$2] - pointSum[i.$1][i.$2]);
  }
}