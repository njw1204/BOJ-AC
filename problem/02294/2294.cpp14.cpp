...
int kind[100], dp[10001];

int main() {
  UNSYNC; READTXT;
  DINPUT2(n, k);
  REP(i, n) INPUT(kind[i]);
  sort(kind, kind + n);

  for (int i = kind[0]; i <= k; i++) {
    for (int j = 0; j < n && i - kind[j] >= 0; j++) {
      if (dp[i - kind[j]] || i == kind[j]) dp[i] = dp[i] ? MIN(dp[i], dp[i - kind[j]] + 1) : dp[i - kind[j]] + 1;
    }
  }
