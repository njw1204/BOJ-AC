...
int GetMineCnt(int si, int sj) {
  int s = 0;
  if (si - 1 >= 0 && field[si - 1][sj] == '*') s++;
  if (si + 1 < R && field[si + 1][sj] == '*') s++;
  if (sj - 1 >= 0 && field[si][sj - 1] == '*') s++;
  if (sj + 1 < C && field[si][sj + 1] == '*') s++;
  if (si - 1 >= 0 && sj - 1 >= 0 && field[si - 1][sj - 1] == '*') s++;
  if (si - 1 >= 0 && sj + 1 < C && field[si - 1][sj + 1] == '*') s++;
  if (si + 1 < R && sj - 1 >= 0 && field[si + 1][sj - 1] == '*') s++;
  if (si + 1 < R && sj + 1 < C && field[si + 1][sj + 1] == '*') s++;
  return s;
}
...
int main() {
  ...
  while (1) {
    INPUT2(R, C);
    if (R == 0) break;
    for (int i = 0; i < R; i++) INPUT(field[i]);
    for (int i = 0; i < R; i++) {
      for (int j = 0; j < C; j++) {
        if (field[i][j] == '.')
          field[i][j] = '0' + GetMineCnt(i, j);
        cout << field[i][j];
      }
      cout << endl;
    }
