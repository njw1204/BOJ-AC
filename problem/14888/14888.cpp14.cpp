...
int N, ansMin = INT_MAX, ansMax = INT_MIN;
int num[11];
int opCnt[4];

void BackTrack(int prenum, int pos, int plus, int minus, int mul, int div) {
  if (pos == N - 1) {
    if (plus) {
      ansMin = MIN(ansMin, prenum + num[pos]);
      ansMax = MAX(ansMax, prenum + num[pos]);
    }
    if (minus) {
      ansMin = MIN(ansMin, prenum - num[pos]);
      ansMax = MAX(ansMax, prenum - num[pos]);
    ...
    if (plus)
      BackTrack(prenum + num[pos], pos + 1, plus - 1, minus, mul, div);
    if (minus)
      BackTrack(prenum - num[pos], pos + 1, plus, minus - 1, mul, div);
    if (mul)
      BackTrack(prenum * num[pos], pos + 1, plus, minus, mul - 1, div);
    if (div)
      BackTrack(prenum / num[pos], pos + 1, plus, minus, mul, div - 1);
  }
}
...
int main() {
  ...
  INPUT(N);
  REP(i, N) INPUT(num[i]);
  REP(i, 4) INPUT(opCnt[i]);
  BackTrack(num[0], 1, opCnt[0], opCnt[1], opCnt[2], opCnt[3]);
  PRINT(ansMax);
  PRINT(ansMin);
  return 0;
}