...
int main() {
  ...
  REP(i, N) {
    ...
    INPUT3(student[i].korean, student[i].english, student[i].math);
  }
    
  sort(student, student + N, [](const auto& a, const auto& b) {
  ...
  FOR(i, 0, N - 1) PRINT(student[i].name);
  RET;
}