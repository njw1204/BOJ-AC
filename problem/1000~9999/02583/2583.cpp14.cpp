...
int BFS(int si, int sj) {
  ...
  while (!Q.empty()) {
    ...
    for (int k = 0; k < 4; k++) {
      if (i.$1 + dx[k].$1 >= 1 && i.$1 + dx[k].$1 <= N &&
        i.$2 + dx[k].$2 >= 1 && i.$2 + dx[k].$2 <= M &&
        field[i.$1 + dx[k].$1][i.$2 + dx[k].$2] && !visit[i.$1 + dx[k].$1][i.$2 + dx[k].$2]) {
        Q.push({ i.$1 + dx[k].$1, i.$2 + dx[k].$2 });
        visit[i.$1 + dx[k].$1][i.$2 + dx[k].$2] = true;
      }
    }
  }
