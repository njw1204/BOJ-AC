...
vector<pair<int, int>> nextNode[10005][2];
...
int DFS(int start, int state) {
  ...
  int asdf = 0;
  if (state) asdf += weight[start];

  for (int i : child[start]) {
    int left = DFS(i, 0), right = 0;
    if (state == 0) right = DFS(i, 1);

    if (left == right && left == 0) {
      continue;
    }
    if (left > right) {
      nextNode[start][state].pb({ i,0 });
      asdf += left;
    }
    else {
      nextNode[start][state].pb({ i,1 });
      asdf += right;
    }
  }

  return dp[start][state] = asdf;
}

void DFSInit(int start, int now_height) {
  visit[start] = true;

  for (int i : link[start]) {
    if (visit[i]) continue;
    child[start].pb(i);
    DFSInit(i, now_height + 1);
  }
}

void PrintAnswer(int start, int state) {
  if (state) answerVector.pb(start);
  for (auto i : nextNode[start][state]) {
    PrintAnswer(i.$1, i.$2);
  }
}
