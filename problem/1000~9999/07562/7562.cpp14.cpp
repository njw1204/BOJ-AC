...
int main() {
  ...
  TESTCASE(T) {
    ...
    while (!Q.empty() && !breakAll) {
      ...
      if (i.$1 == ei && i.$2 == ej) {
        cout << visit[ei][ej] - 1 << endl;
        breakAll = true;
        break;
      }

      for (auto j : dx) {
        if (BETWEEN(i.$1 + j.$1, 0, L - 1) && BETWEEN(i.$2 + j.$2, 0, L - 1) && !visit[i.$1 + j.$1][i.$2 + j.$2]) {
          Q.push({ i.$1 + j.$1, i.$2 + j.$2 });
          visit[i.$1 + j.$1][i.$2 + j.$2] = visit[i.$1][i.$2] + 1;
          if (i.$1 + j.$1 == ei && i.$2 + j.$2 == ej) {
            cout << visit[ei][ej] - 1 << endl;
            breakAll = true;
            break;
          }
