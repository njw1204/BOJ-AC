...
#define FOR(i,from,to) for(auto i=(to)*0+(from);((to)>=(from))?(i<=(to)):(i>=(to));i+=((to)>=(from))?1:-1)
#define FORS(i,from,to,step) for(auto i=(to)*0+(step)*0+(from);((to)>=(from))?(i<=(to)):(i>=(to));i+=(step))
