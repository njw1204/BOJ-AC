...
const char special[3][4] = {"dz=","lj","nj"};
const char space[2][4] = {"   ", "  "};
...
int main() {
  ...
  for (auto i : special) {
    size_t pos = -1;
    while ((pos = T.find(i, pos + 1)) != string::npos) {
      ans++;
      T.replace(pos, strlen(i), (i[0] == 'd' ? space[0] : space[1]));
    }
  }
  ...
  for (auto i : alphabet) {
    if (i) {
      size_t pos = -1;
      while ((pos = T.find(i, pos + 1)) != string::npos) ans++;
    }
  }
