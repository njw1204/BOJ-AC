...
int Dijkstra() {
  auto cmp = [](pair<int,int> a, pair<int,int> b) { return field[a.$1][a.$2] > field[b.$1][b.$2]; };
  ...
  while (!Q.empty()) {
    ...
    for (auto j : dx) {
      int ni = i.$1 + j.$1, nj = i.$2 + j.$2;
      if (ni >= 1 && ni <= N && nj >= 1 && nj <= N) {
        if (visit[i.$1][i.$2] + field[ni][nj] < visit[ni][nj]) {
          visit[ni][nj] = visit[i.$1][i.$2] + field[ni][nj];
          Q.push({ni,nj});
        }
      }
    }
